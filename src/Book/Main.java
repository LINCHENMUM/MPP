...
//		allEntries.stream()
//		          .filter(e->e.getDueDate().equals(LocalDate.of(2016,2,3)))
//		          .sorted(Comparator.comparing(ChecoutRecordEntry::getCheckoutDate))
//		          .thenComparing(Comparator.comparing(CheckoutRecord::getDueDate))
...
//		          .forEach(System.out::println);
		...
		String titles=allBooks.stream()
				              .filter(b->b.getGenre()==Genre.ROMANCE)
...
//		.reduce(Double.MAX_VALUE,Double::min));
	...
	System.out.println(allEntries.stream()
		          .map(ChecoutRecordEntry::getBook)
		          .map(Book::getPrice)
		          .reduce(Double.MAX_VALUE,(a,b)->Double.min(a, b))
		          //.collect(Collectors.joining())
		          //.forEach(System.out::print);
		          .doubleValue());
	Optional<Book> smallPrice=allEntries.stream()
			                            .map(ChecoutRecordEntry::getBook)
			                            .reduce((b1,b2)->b1.getPrice()<b2.getPrice()?b1:b2);
	System.out.println(smallPrice.get());
